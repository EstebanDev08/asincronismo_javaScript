...
/** XMLHttpRequest es un objeto de JS que permite hacer
 * peticiones hacia servicios en la nube(URLs o APIs).*/
...
function fetchData(urlApi, callback) {
  ...
  xhttp.onreadystatechange = function (event) {
      if (xhttp.readyState === 4) {        /** 4 = "la peticion se completo correctamente"*/

          if (xhttp.status === 200) {      /** 200 = " Indican que la petición fue recibida, aceptada y procesada correctamente."*/
            callback(null, JSON.parse(xhttp.responseText));
          }else {
              const error = new Error(`Error ${xhttp.status} : ${xhttp.statusText} ' api: ${urlApi}'`);
